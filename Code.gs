...
var SHEET_URL = "https://docs.google.com/spreadsheets/d/your-id-here/edit";
...
function set_sheet_headers() {
  ...
  sheet.appendRow(["Audit_Date","Inbox_Threads","Inbox_Unread_Count","Priority_Inbox_Threads", "Priority_Inbox_Unread_Count", "Spam_Threads", "Spam_Unread_Count", "Last_Hour_Count", "Not_Inbox_Unread","Last_Hour_Unsubscribe_Count","last_hour_from_my_domain","last_hour_not_from_my_domain"]);
...
function getGmailStats() {
  ...
  var last_hour_from_my_domain = countQuery('newer_than:1h from:gmail.com ');
  var last_hour_not_from_my_domain = countQuery('newer_than:1h !from:gmail.com ');

    //push data in as a multiple columns
  var ss = SpreadsheetApp.openByUrl(SHEET_URL).getSheetByName(MAIN_SHEET_TAB_NAME);
  ss.appendRow([audit_date,inbox_threads,inbox_unread_count,priority_inbox_threads, priority_inbox_unread_count, spam_threads, spam_unread_count, last_hour_count, not_inbox_unread,last_hour_unsubscribe_count,last_hour_from_my_domain,last_hour_not_from_my_domain]);
